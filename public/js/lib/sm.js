 ...
 * =====================================================
 * 
 * 必须调用$.init()方法, 他会调用$.initPage方法初始化页面组件, 并触发一个pageInit事件
 * $.config.autoInit=true, 那么document.onload的时候执行顺序为 : pageInit事件绑定 --> $.init --> $initPage --> pageInit();
 * $.config.autoInit=false, 执行顺序为 : pageInit事件绑定 --> $.init --> $initPage --> pageInit();
 * 
 * +++ 路由功能:
 * 	> 每一个页面.page容器都应该有一个全局唯一的id, 路由器会将其作为唯一标示, 如果没有设置id. 如果写这样的一个链接就可以跳转到内联页面<a href='#{page-id}'>跳转</a>
 * 
 * +++ 缓存
 * 	> 对于加载过的页面, 只要页面没有刷新, 那么再次进入该页面的时, 都会优先读取缓存而不发送网络请求, 如果需要强制请求, 可以用下面两个方式:
 * 		- 在a链接上添加 data-no-cache="true" 属性来标志点击该链接时不使用缓存, 如:<a href="/detail" data-no-cache="true">no cache</a>
 *  	- 使用js方法load来加载时, 把可选的参数ingoreCache设为true, 如: $.router.load('/detail', true)
 *  	- 
 * +++ JS方法
 * 	> 可以通过调用$.router.load(url, ignoreCache)来加载一页面, 参数url可以是一个ajax页面的地址, 也可以是一个内联页面的id
 * 		$.router.load("/detail");  //加载ajax页面
   ...
   +++ 事件
   	> 一个页面准备开始加载, 到执行完加载动画, 会按顺序触发一下事件
   		事件				说明
...
;$.smVersion = "0.6.2";+function ($) {
    ...
    //全局配置
    var defaults = {
        autoInit: false, //自动初始化页面, 如果页面内容是异步加载, 应当关闭这个配置, 并且在加载完毕后手动调用$.init 
        showPageLoadingIndicator: true, //push.js加载页面的时候显示一个加载提示
        //如果想走原生的跳转而不使用router, 可以在连接上添加class="external", 例如:<a href="xxx" external>xxx</a>
        router: true, //默认使用router, 想关闭这个功能, 在页面加载MSUI之前配置$.config.router = false;
...
//            $.toast('加载失败');
    ...
    $.init = function() {
        var $page = getPage();
        var id = $page[0].id;
        $.initPage();//调用$.initPage()方法初始化页面
        $page.trigger('pageInit', [id, $page]);//调用pageInit事件
    };
